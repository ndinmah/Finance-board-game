...
export default function WinnerModal() {
  const { players, winnerId, myPlayerId, loadDevState } = useGameStore();
  const winner = players.get(winnerId);
  const isMe = winnerId === myPlayerId;
  const handleReturn = () => {
    if (IS_DEV_MODE) loadDevState();
    else leaveRoom();
  };
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-br from-[#1e2d5a] to-[#162040] border-[0.1333rem] border-[rgba(245,197,24,0.5)] rounded-[1.0667rem] md:rounded-[1.6rem] p-[2.1333rem_1.3333rem] md:p-[3.2rem_2.6667rem] text-center w-[90%] max-w-[25.3333rem] shadow-[0_0_4rem_rgba(245,197,24,0.2),0_1.6rem_4.2667rem_rgba(0,0,0,0.7)] animate-winner-slide">
        ...
        <button id="btn-return-lobby" className="font-inter cursor-pointer touch-manipulation disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.96] w-full md:w-auto bg-gradient-to-br from-[#4a90d9] to-[#2176ae] text-white text-[1rem] md:text-[1.0667rem] font-bold p-[0.8rem_1.6rem] md:p-[0.9333rem_2.1333rem] rounded-[0.9333rem] border-none cursor-pointer shadow-[0_0.2667rem_1.0667rem_rgba(74,144,217,0.4)] transition-all duration-150 hover:-translate-y-[0.1333rem] hover:shadow-[0_0.5333rem_1.6rem_rgba(74,144,217,0.6)] mt-[1.3333rem]" onClick={handleReturn}>
